...
// The browser cannot import this Node script, so the vocabulary is emitted as
// data. One source of truth: TECH below, written out on every fetch.
const VOCAB_OUT = 'src/data/vocabulary.json'
const TARGET = 120
...
/**
 * Emit the vocabulary as data so the app resolves tokens through the exact same
 * table the fetcher tagged with. Duplicating this in the UI would let the two
 * drift, and a drifted alias table produces false gaps — the failure this layer
 * exists to prevent.
 */
async function writeVocabulary() {
  ...
  for (const [canonical, spec] of Object.entries(TECH)) {
    concepts[canonical] = {
      ...
      profileAliases: spec.profileAliases ?? [],
    }
    aliasToCanonical[canonical.toLowerCase()] = canonical
    for (const a of spec.aliases ?? []) aliasToCanonical[a.toLowerCase()] = canonical
    for (const a of spec.profileAliases ?? []) aliasToCanonical[a.toLowerCase()] = canonical
  }
  await writeFile(VOCAB_OUT, JSON.stringify({
    note: 'Generated by scripts/fetch-jobs.mjs. Do not edit by hand.',
    coreMl: CORE_ML,
    hyphenGuarded: [...HYPHEN_GUARDED],
    concepts,
    aliasToCanonical,
  }, null, 2), 'utf8')
  console.log(`  vocabulary -> ${VOCAB_OUT} (${Object.keys(concepts).length} concepts, ${Object.keys(aliasToCanonical).length} aliases)`)
}
